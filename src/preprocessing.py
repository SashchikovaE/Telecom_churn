...
class DataPreprocessor:
    ...
    def __init__(self, df=None):
        ...
        Args:
            filepath (str): Path to the CSV file containing the raws data.
        """
        if df is None:
    ...
    def visualize_outliers(self):
        ...
        file = Path(__file__).parent.parent / 'images/outliers_boxplot.png'
    ...
    def visualize_churn_distribution(self):
        ...
        file = Path(__file__).parent.parent / 'images/churn_distribution.png'
        file.parent.mkdir(exist_ok=True)
        plt.savefig(file)
        plt.show()
        '''
    ...
    def move_target_to_end(self):
        if 'Churn' in self.df.columns:
            target = self.df['Churn']
            self.df = self.df.drop('Churn', axis=1)
            self.df['Churn'] = target
